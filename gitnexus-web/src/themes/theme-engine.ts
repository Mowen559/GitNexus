...
export function applyTheme(config: ThemeConfig): void {
  ...
  // 1b. Map UA preset tokens onto GitNexus-specific base aliases so the
  // existing GN chrome (body background, hover surfaces, borders) also
  // responds to theme changes. GN uses --color-void/--color-deep/--color-hover
  // and --color-border-default which UA presets don't define directly.
  style.setProperty('--color-void', preset.colors.root);
  style.setProperty('--color-deep', preset.colors.surface);
  style.setProperty('--color-hover', preset.colors.elevated);
  style.setProperty('--color-border-default', preset.isDark ? '#2a2a3a' : '#d4cfc8');
  // GN node color aliases (folder/method/interface/import) not in UA presets.
  if (preset.colors['node-module'])
    style.setProperty('--color-node-folder', preset.colors['node-module']);
  style.setProperty('--color-node-method', preset.colors['node-function'] ?? '#14b8a6');
  style.setProperty('--color-node-interface', preset.colors['node-class'] ?? '#ec4899');
  style.setProperty('--color-node-import', preset.colors['text-muted'] ?? '#6b7280');
